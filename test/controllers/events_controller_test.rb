...
class EventsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "user go to home page when logout" do
   ...
   #Maybe check if submot button is disabled when dont login
   test "should be attendee" do
      @my_event=events(:madrid_rio)
      login_as(@user)
      get "/events/#{@my_event.id}"
      assert_response :success
      #post rsvps_url, params: { event: { address: @event.address, description: @event.description, end_at: @event.end_at, photo_file_name: @event.photo_file_name, name: @event.name, start_at: @event.start_at } }
      #follow_redirect!
      Warden.test_reset! #reset Warden after each example
   end
